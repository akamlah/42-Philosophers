...
/*   Updated: 2022/01/13 11:34:42 by alicekamlah      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ph_parse(t_ph_vars *phbx, int argc, char **argv)
{
	...
	if (!ph_only_positive_integers_as_args(argv))
		return (-1);
	phbx->number_of_philosophers = ft_atoi(argv[1]);
	if (phbx->number_of_philosophers == 0)
	{
		printf("Error: 'numer_of_philosophers' has to be at least 1.\n");
		return (-1);
	}
	phbx->time_to_die = ft_atoi(argv[2]);
	phbx->time_to_eat = ft_atoi(argv[3]);
	phbx->time_to_sleep = ft_atoi(argv[4]);
	if (argv[5])
		phbx->number_of_times_each_philosopher_must_eat = ft_atoi(argv[5]);
	else
		phbx->number_of_times_each_philosopher_must_eat = -1;
	return (0);
}
